...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist &artist) const
{
    ...
    //go to the location in the graph based on artist
    Vertex *current = graph.at(artist.get_name());
    
    //Push each of the artists neighbors onto the vector of artists
    for(int i = 0; i < (int)(current->neighbors.size()); i++)
    {
        result.push_back(current->neighbors[i].neighbor);
    }
    
    //return vector of artists
    return result;
}
                                            ...
                                            const Artist &dest) const
{
    ...
    //go to location of the graph based on destination artist
    Vertex *current = graph.at(dest.get_name());
    
    //if the path is empty, return an empty stack
    if(current->predecessor == nullptr)
    {
        return path;
    }
    
    //push collaborators onto stack starting from the back working towards
    //source
    while(current->artist != source)
    {
        path.push(current->artist);
